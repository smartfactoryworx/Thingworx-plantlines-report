...
@Component({
  selector: 'app-cycle-report-old',
  templateUrl: './cycle-report-old.component.html',
  styleUrls: ['./cycle-report-old.component.scss']
})

export class CycleReportOldComponent implements OnChanges {
  ...
  GetCycleData(machineDetails) {
    ...
    this.dataentryservice.GetApiURL().subscribe(apipath => {
      ...
      this.dataentryservice.GetMachineDetails(apipath['apithings'], dataSource, JSON.stringify(body)).subscribe((machinecycledata: any) => {
        ...
        for (let i = 0; i < c.length; i++) {
          ...
          const allCycleData = {
            ...
            From: currentData && moment(Number(currentData.StartTime)).utcOffset(330).format('HH:mm'),
            ...
            To: currentData && moment(Number(currentData.StopTime)).utcOffset(330).format('HH:mm'),
            Date: currentData && moment(Number(currentData.StartTime)).utcOffset(330).format('DD MMM YYYY'),
